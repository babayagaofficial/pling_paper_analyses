    ...
    def _make_nucmer_files(self, outprefix):
        ...
        for i in range(len(self.assemblies) - 1):
            ...
            if os.path.exists(nucmer_file):
                ...
                subprocess.check_call(f"nucmer --diagdiff 20 --breaklen 500  --maxmatch {self.fasta_files[i+1]} {self.fasta_files[i]} && delta-filter -1 out.delta > out.1delta", shell=True)
                subprocess.check_call(f"show-coords -Tdl -I {self.nucmer_min_id} out.1delta > {nucmer_file}", shell=True)
...
parser = argparse.ArgumentParser(
    description = 'Makes cartoon ACT-style figure comparing at least two FASTA files. Files are shown from top to bottom in the same order as listed in the text file given to this command.',
    usage = '%(prog)s [options] <outprefix> <fa_list>')
...
parser.add_argument('fa_list', help='Text file with list of paths to at least 2 fasta files')
options = parser.parse_args()


fa_list = [el[0] for el in pd.read_csv(options.fa_list, header=None).values]
